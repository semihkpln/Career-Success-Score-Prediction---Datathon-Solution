...
# ══════════════════════════════════════════════════════════════════════════════
# 0. NLP MODELİ HAZIRLIĞI (BERTürk)
# ══════════════════════════════════════════════════════════════════════════════
print(SEP)
print("0.  BERTürk NLP MODELİ YÜKLENİYOR...")
print(SEP)
...
# Modellerin kendi içinde kullanacağı kategorik sütunlar
CAT_COLS = [
    "target_role", "department", "hobby", "preferred_social_media_platform"
]

# Artık CAT_COLS'u silmiyoruz! Onlar da veride kalacak.
DROP_COLS_UPDATED = [
...
def engineer_features(df: pd.DataFrame, ref: pd.DataFrame = None) -> pd.DataFrame:
    ...
    for col in CAT_COLS:
        if col in df.columns:
            # Boşlukları doldur ve string olduğundan emin ol (CatBoost için zorunlu)
            df[col] = df[col].fillna("Unknown").astype(str)
            # Metin okuyamayan modeller (ExtraTrees vb.) için Frekans Kodlaması yap
    ...
    for col in ["english_exam_score", "github_avg_stars", "portfolio_score"]:
        df[col] = df[col].fillna(ref[col].mean())
    ...
    # ── YENİ: Güçlü Etkileşim Özellikleri ───────────────────────────────────────
    ...
    # ── Gereksiz Sütunları Kaldır (CAT_COLS artık silinmiyor) ────────────────
...
# Target encoding sütunlarını ekle
...
def oof_predict(model, X: pd.DataFrame, y: pd.Series, X_test: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    # Modellerin türüne göre veri setini dinamik olarak ayarla
    if model_name in ["ExtraTreesRegressor", "Ridge", "XGBRegressor"]:
        # String/Object sütunları desteklemeyen modeller için bu sütunları at
        X_model = X.select_dtypes(exclude=['object', 'string', 'category'])
        X_test_model = X_test.select_dtypes(exclude=['object', 'string', 'category'])
    else:
        # CatBoost ve LightGBM için verinin kopyasını al
        X_model = X.copy()
        X_test_model = X_test.copy()

        # LightGBM özel olarak 'category' veri tipi ister
...
def cat_obj(trial):
    p = dict(
        ...
        cat_features=CAT_COLS  # <-- BURAYA EKLENDİ
    )
...
# CatBoost'un nihai modeline de cat_features ekledik
...
print(f"\n  ✓ Tahminler → 'submission_2.1.csv' dosyasına kaydedildi!")
print(SEP)